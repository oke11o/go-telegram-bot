...
func NewStartChooseTournament(deps *fsm.Deps) *StartChooseTournament {
	return &StartChooseTournament{
		Base: Base{base.Base{Deps: deps}},
	}
}

type StartChooseTournament struct {
	Base
}

func (m *StartChooseTournament) Switch(ctx context.Context, state fsm.State) (context.Context, fsm.Machine, fsm.State, error) {
